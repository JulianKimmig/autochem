...
class AbstractNMRtester:
    ...
    def _raw_peaks(self):
        d = self._norm()
        peaks, peak_data = find_peaks(y=d["norm"]["data"], x=d["zoom"]["ppm"], min_peak_height=0.05, rel_height=0.001,
                                      max_width=5
                                      )
    ...
    def _find_peaks(self):
        d = self._zoom_peaks()
        peaks, peak_data = find_peaks(y=d["zoom_peaks"]["data"], x=d["zoom_peaks"]["ppm"], min_peak_height=0.1,
                                      min_distance=0.1, rel_height=0.01, rel_prominence=0.4, center="median")
        d["find_peaks"] = {"peaks": peaks, "peak_data": peak_data}
        return d

    def _integrate_peaks(self):
        d = self._find_peaks()
        peaks, peak_data = peak_integration(x=d["zoom_peaks"]["ppm"],
                                            y=d["zoom_peaks"]["data"],
    ...
    def _ref_peaks(self):
        ...
        pidx, peak = get_reference_peak(
            d["zoom_peaks"]["ppm"][d["integrate_peaks"]["peaks"]],
            ref_peak,
            max_diff=1)
    ...
    def test_03_rubberband(self):
        ...
        if PLOT:
            ...
            plt.plot(ppm[corr_data['points']], d["rubberband"]["data"][corr_data['points']], "o",
    ...
    def test_05_peaks(self):
        ...
        if PLOT:
            ...
            plt.plot(y[np.floor(d["raw_peaks"]["peak_data"]['left_ips']).astype(int)],
                     x[np.floor(d["raw_peaks"]["peak_data"]['left_ips']).astype(int)], "o", label="left_ips")
            plt.plot(y[np.floor(d["raw_peaks"]["peak_data"]['right_ips']).astype(int)],
                     x[np.floor(d["raw_peaks"]["peak_data"]['right_ips']).astype(int)], "+", label="right_ips")
            plt.title("peaks")
            plt.legend()
            plt.show()
            plt.close()
    ...
    def test_07_find_peaks(self):
        ...
        if PLOT:
            ...
            x = d["zoom_peaks"]["data"]
            plt.plot(y, x, label="data")
            plt.plot(y[d["find_peaks"]["peaks"]], d["find_peaks"]["peak_data"]['peak_heights'], "o", label="peaks")
            plt.plot(y[d["find_peaks"]["peak_data"]['peak_left_border']],
                     x[d["find_peaks"]["peak_data"]['peak_left_border']], "*", label="left_ips")
            plt.plot(y[d["find_peaks"]["peak_data"]['peak_right_border']],
                     x[d["find_peaks"]["peak_data"]['peak_right_border']], "+", label="right_ips")
    ...
    def test_09_ref_peaks(self):
        ...
        assert np.allclose(round_ints, self.NORMED_PEAKS), [round_ints, self.NORMED_PEAKS]
...
class TestNMRMakgritek(AbstractNMRtester, unittest.TestCase):
    ...
    PEAK_DIFF = [0.2562199, -0.13086617, 0., -0.24022015, -0.34425704]
...
class TestNMRBruker(AbstractNMRtester, unittest.TestCase):
    ...
    PEAK_DIFF = [-0.46440374, -0.15092486, 0.25956847, 0., 0.23210654]
    TESTFILE = os.path.join(os.path.dirname(__file__), "testdata", "nmr", "bruker_1", "10")
